...
if __name__ == '__main__':
    ...
    # args = DotMap(dict(benchmark=False, distributed=False, local_rank=0, rank=0, test=False, tf=False, train=True))
    # model = EfficientNet(args)
    # model = torch.load('weights/best_pt.pt', map_location='cpu')['model'].float().eval()
    model = torch.load('/Users/yang/01_code/EffcientNetV2/weights/best_pt.pt', map_location='cpu')['model'].float().eval()
    # model = torch.load("", map_location='cpu')['model'].float().eval()
